    ...
    @foreach ($items as $route => $params)
        ...
        <li role="presentation" {!! Html::classes(['active' => Route::is($route)]) !!}>
            @if (isset($params['submenu']) && count($params['submenu']) > 0)
                ...
                <ul id="{{$params['submenu_id']}}" class="collapse">
                    @foreach ($params['submenu'] as $sm)
                        <li>
                            <a href="{{ route($sm['url']) }}"><i class="fa fa-fw {{ $sm['icon'] }}"></i>  {{ $sm['title'] }}</a>
                        </li>
                    @endforeach
                </ul>
            @else
                <a href="{{ route($route) }}"><i class="fa fa-fw {{ $params['icon'] }}"></i> {{ $params['title'] }}</a>
            @endif
        </li>
    @endforeach
